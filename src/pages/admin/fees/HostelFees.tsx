...
import { TrendingUp, Users, AlertCircle, Loader2, X, Search, Bed } from 'lucide-react';
...
const stMap: Record<string, { bg: string; color: string; label: string }> = {
  ...
  not_applicable: { bg: '#f1f5f9', color: '#94a3b8', label: 'N/A'  }
};
...
export default function HostelFees() {
  ...
  const { students, refetch: refetchStudents, loading: stdLoading, error: stdError } = useStudents(schoolId, { session_id: currentSession?.id });
  const { updateStudent, loading: updatingStudent } = useUpdateStudent();
  ...
  const tableData = useMemo(() => {
    ...
    collections.forEach(col => {
      ...
      const hasHostelAdmission = types.includes('hostel admission');
      const hasHostelMonthly = types.includes('hostel monthly');
      
      const stCustom = paidCustomFieldsMap.get(col.student_id) || new Set();
      let hasCustom = false;
      
      types.forEach(t => {
        if (t && t !== 'hostel admission' && t !== 'hostel monthly' && !MONTHS.map(m=>m.toLowerCase()).includes(t)) {
          stCustom.add(t);
          hasCustom = true;
        }
      });
      
      if (!hasHostelAdmission && !hasHostelMonthly && !hasCustom) return;
      
      const existing = paidMap.get(col.student_id) || 0;
      paidMap.set(col.student_id, existing + Number(col.amount_paid));

      if (hasHostelAdmission) {
      ...
      if (hasHostelMonthly && col.fee_months) {
    ...
    return students.map(st => {
      ...
      const hostelFeesCfg = cls?.raw_details?.hostel_fees || {};
      
      const adminFee = Number(hostelFeesCfg.admission_fee) || 0;
      const monFee = Number(hostelFeesCfg.monthly_fee) || 0;
      const additionalFees = hostelFeesCfg.additional_fees || {};
      ...
      // If student is not explicitly marked as hosteler we consider them Not Applicable.
      const isHosteler = st.is_hosteler === true;
      const willApply = isHosteler;
      
      let status = 'unpaid';
      if (!willApply || totalFee === 0) {
        status = 'not_applicable'; // Fees haven't been configured or student isn't in hostel
  ...
  const filteredData = useMemo(() => {
    return tableData.filter(d => {
      if (selectedStatus !== 'All Status') {
        if (selectedStatus === 'Not Applicable' && d.status !== 'not_applicable') return false;
        if (selectedStatus !== 'Not Applicable' && d.status.toLowerCase() !== selectedStatus.toLowerCase()) return false;
      }
      ...
      if (search) {
        ...
        if (!fullName.includes(query) && !admNo.includes(query) && !rollNo.includes(query) && !matchesReceipt) {
          return false;
        }
      }
      return true;
    });
  }, [tableData, selectedClass, selectedStatus, selectedSection, search]);

  const summary = useMemo(() => {
    let totalDue = 0, collected = 0, paidSt = 0, pendingSt = 0;
    tableData.forEach(d => {
      if (d.status === 'not_applicable') return;
      totalDue += d.totalFee;
      collected += d.paid;
  ...
  const openModal = (st: any) => {
    if (st.status === 'not_applicable') return;
    setActiveStudent(st);
  ...
  const handleCollect = async () => {
    ...
    if (payAdmission) { amount += activeStudent.adminFee; typeParts.push('Hostel Admission'); }
    if (payMonthly) { 
  ...
  return (
    <AdminLayout pageTitle="Hostel Fees" pageSubtitle="Track and manage hostel fee collections">
      {globalError ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 22 }}>
            {[
              { label: 'Expected Total (Hostel)', value: `₹${summary.totalDue.toLocaleString()}`, icon: Bed, color: '#7c3aed', bg: '#ede9fe' },
              { label: 'Collected', value: `₹${summary.collected.toLocaleString()}`, icon: TrendingUp, color: '#16a34a', bg: '#dcfce7' },
              { label: 'Paid Hostelers', value: summary.paidSt, icon: Users, color: '#0891b2', bg: '#cffafe' },
              { label: 'Pending Hostelers', value: summary.pendingSt, icon: AlertCircle, color: '#dc2626', bg: '#fee2e2' },
            ].map(c => {
              const Icon = c.icon;
              return (
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: '#1e293b', flexShrink: 0 }}>Hostel Fee Records — {currentSession?.name || 'Current'}</span>
              ...
              <select value={selectedStatus} onChange={e => setSelectedStatus(e.target.value)} style={{ padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: 9, fontSize: 12, color: '#475569', outline: 'none', fontFamily: 'inherit', background: '#fff' }}>
                ...
                <option>Paid</option><option>Partial</option><option>Unpaid</option><option>Not Applicable</option>
              </select>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#f8fafc' }}>
                      {['Student', 'Hosteler?', 'Class', 'Total Due', 'Paid', 'Bal. Due', 'Status', 'Actions'].map(h => (
                  ...
                  <tbody>
                    {filteredData.map((f, i) => {
                      ...
                      return (
                        <tr key={f.id} style={{ borderTop: '1px solid #f1f5f9', transition: 'background 0.2s', opacity: isNA ? 0.7 : 1 }}
                          ...
                          <td style={{ padding: '12px 16px' }}>
                             <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', opacity: f.totalFee === 0 ? 0.4 : 1 }}>
                               <input 
                                 type="checkbox"
                                 checked={!!f.is_hosteler}
                                 disabled={updatingStudent || f.totalFee === 0}
                                 onChange={async (e) => {
                                   if (!schoolId) return;
                                   const checked = e.target.checked;
                                   await updateStudent(f.id, schoolId, { is_hosteler: checked });
                                   refetchStudents();
                                 }}
                                 style={{ width: 16, height: 16, accentColor: '#7c3aed', cursor: 'inherit' }}
                               />
                             </label>
                          </td>
                          <td style={{ padding: '12px 16px', fontSize: 13, color: '#475569' }}>{f.class_name} {f.current_section && `(${f.current_section})`}</td>
                          <td style={{ padding: '12px 16px', fontSize: 13, color: '#1e293b', fontWeight: 600 }}>{isNA ? '—' : `₹${f.totalFee.toLocaleString()}`}</td>
                          <td style={{ padding: '12px 16px', fontSize: 13, color: '#16a34a', fontWeight: 600 }}>{isNA ? '—' : `₹${f.paid.toLocaleString()}`}</td>
                          <td style={{ padding: '12px 16px', fontSize: 13, color: f.due > 0 ? '#dc2626' : '#94a3b8', fontWeight: 600 }}>{isNA ? '—' : `₹${f.due.toLocaleString()}`}</td>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div style={{ display: 'flex', gap: 6 }}>
                              <button 
                                onClick={() => openModal(f)}
                                disabled={isNA}
                                style={{ padding: '5px 12px', background: isNA ? '#f1f5f9' : '#ede9fe', border: 'none', borderRadius: 7, fontSize: 12, color: isNA ? '#94a3b8' : '#7c3aed', cursor: isNA ? 'not-allowed' : 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>
                                Collect
                              </button>
                              <button 
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: '#fff', borderRadius: 16, width: 400, boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 16, color: '#1e293b' }}>Collect Hostel Fee</h3>
            ...
            <div style={{ padding: 24 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', border: '1px solid #e2e8f0', borderRadius: 8, marginBottom: 12, cursor: activeStudent.hasPaidAdmission ? 'not-allowed' : 'pointer', background: activeStudent.hasPaidAdmission ? '#f8fafc' : '#fff' }}>
                ...
                />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: activeStudent.hasPaidAdmission ? '#94a3b8' : '#1e293b' }}>Hostel Admission {activeStudent.hasPaidAdmission && '(Paid)'}</div>
                </div>
              ...
              <div style={{ padding: '12px 16px', border: '1px solid #e2e8f0', borderRadius: 8, marginBottom: 12 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: activeStudent.monFee === 0 ? 'not-allowed' : 'pointer' }}>
                  ...
                  />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1e293b' }}>Hostel Monthly {payMonthly && selectedMonths.length > 0 && `(x${selectedMonths.length})`}</div>
                  </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: '#fff', borderRadius: 16, width: 450, maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: '#fff' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 16, color: '#1e293b' }}>Hostel Receipts History</h3>
            ...
            <div style={{ padding: 20 }}>
              {collections.filter(c => c.student_id === historyStudent.id && c.fee_type?.toLowerCase().includes('hostel')).length === 0 ? (
                <div style={{ textAlign: 'center', color: '#94a3b8', padding: 20, fontSize: 13 }}>No hostel payment records found.</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {collections.filter(c => c.student_id === historyStudent.id && c.fee_type?.toLowerCase().includes('hostel')).map(r => (
