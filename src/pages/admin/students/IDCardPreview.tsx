...
export default function IDCardPreview({ student, school }: IDCardPreviewProps) {
  ...
  const photoContainerStyle: React.CSSProperties = {
    width: '10mm', // Reduced from 24mm
    height: '12mm', // Reduced from 32mm
    margin: '3px auto 0', // Reduced from 8px
    borderRadius: '8px',
    padding: '1px', // Reduced from 1.5px
  ...
  const labelValueStyle: React.CSSProperties = {
    display: 'grid',
    gridTemplateColumns: 'minmax(40px, 1fr) 1.8fr',
    width: '100%',
    gap: '2px', // Reduced from 4px
    fontSize: '7px', // Reduced from 7.5px
    marginBottom: '1.5px', // Reduced from 3px
    alignItems: 'baseline',
  };
  ...
  const labelStyle: React.CSSProperties = {
    ...
    fontSize: '6.5px', // Reduced from 7px
    ...
    <div className="id-card-printable" style={cardStyle}>
      ...
      <div style={watermarkStyle}>
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <pattern id="pattern-hex" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
      ...
      <div style={photoContainerStyle}>
        <div style={photoInnerWrapperStyle}>
          {student.photo_url ? (
            <img src={student.photo_url} alt="Student" style={studentPhotoStyle} />
          ) : (
            <div style={{ ...studentPhotoStyle, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={infoSectionStyle}>
        ...
        <div style={labelValueStyle}>
          ...
          <span style={valueStyle}>{student.erp_classes?.name || 'N/A'} {student.current_section ? `(${student.current_section})` : ''}</span>
        </div>
        <div style={labelValueStyle}>
          <span style={labelStyle}>Roll No</span>
          <span style={valueStyle}>{student.roll_number || '-'}</span>
        </div>
        <div style={labelValueStyle}>
          <span style={labelStyle}>D.O.B</span>
          <span style={valueStyle}>{student.date_of_birth ? new Date(student.date_of_birth).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}</span>
        </div>
        <div style={labelValueStyle}>
          <span style={labelStyle}>Blood Gp</span>
          <span style={valueStyle}>{student.blood_group || '-'}</span>
        </div>
        <div style={labelValueStyle}>
          <span style={labelStyle}>Father</span>
          <span style={{ ...valueStyle, fontSize: '8px' }}>{student.father_name || '-'}</span>
        </div>
        <div style={labelValueStyle}>
          <span style={labelStyle}>Contact</span>
          <span style={valueStyle}>{student.parent_contact || student.guardian_contact || '-'}</span>
        </div>
        <div style={{ ...labelValueStyle, marginTop: '2px' }}>
          <span style={labelStyle}>Address</span>
          <span style={{ ...valueStyle, fontSize: '7.5px', lineHeight: 1.1, fontWeight: 600, color: '#475569' }}>{student.address || '-'}</span>
        </div>
      </div>
